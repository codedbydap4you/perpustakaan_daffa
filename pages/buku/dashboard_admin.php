...
// Query untuk mendapatkan stok buku yang sedikit (kurang dari 5)
$query_stok_sedikit = "SELECT COUNT(*) as total_stok_sedikit FROM buku WHERE stok < 5";
$result_stok_sedikit = mysqli_query($koneksi, $query_stok_sedikit);
$row_stok_sedikit = mysqli_fetch_assoc($result_stok_sedikit);
$total_stok_sedikit = $row_stok_sedikit['total_stok_sedikit'];
...
// Ganti query untuk mendapatkan aktivitas terbaru
$query_aktivitas = "SELECT * FROM log_aktivitas ORDER BY waktu DESC LIMIT 6";
...
<body>
    ...
    <button id="toggleSidebar">
        ...
        <div class="content">
            <div class="header">
                <h1>DASHBOARD</h1>
                <div class="user-info">
                        <img src="../../../perpustakaan_daffa/img/logo-perpustakaan.png" alt="Logo Perpustakaan">                    <div>
            ...
            <div class="stats-container">
                ...
                <div class="stat-card">
                    ...
                    <h3><?php echo $total_stok_sedikit; ?></h3>
                    <p>Stok Menipis</p>
                </div>
            </div>
    ...
    if (mysqli_num_rows($result_aktivitas) > 0) {
        while ($row_aktivitas = mysqli_fetch_assoc($result_aktivitas)) {
            // Tentukan icon berdasarkan jenis aksi
            $icon = 'fas fa-edit';
            if (strpos($row_aktivitas['aksi'], 'tambah') !== false || strpos($row_aktivitas['aksi'], 'Tambah') !== false) {
                $icon = 'fas fa-plus-circle';
            } else if (strpos($row_aktivitas['aksi'], 'hapus') !== false || strpos($row_aktivitas['aksi'], 'Hapus') !== false) {
                $icon = 'fas fa-trash';
            } else if (strpos($row_aktivitas['aksi'], 'edit') !== false || strpos($row_aktivitas['aksi'], 'Edit') !== false) {
                $icon = 'fas fa-edit';
            } else if (strpos($row_aktivitas['aksi'], 'login') !== false || strpos($row_aktivitas['aksi'], 'Login') !== false) {
                $icon = 'fas fa-sign-in-alt';
            } else if (strpos($row_aktivitas['aksi'], 'logout') !== false || strpos($row_aktivitas['aksi'], 'Logout') !== false) {
                $icon = 'fas fa-sign-out-alt';
            }
            
            // Format waktu
            $timestamp = strtotime($row_aktivitas['waktu']);
            $waktu_sekarang = time();
            $selisih = $waktu_sekarang - $timestamp;
            
            if ($selisih < 60) {
                $waktu_format = "Baru saja";
            } else if ($selisih < 3600) {
                $menit = floor($selisih / 60);
                $waktu_format = $menit . " menit yang lalu";
            } else if ($selisih < 86400) {
                $jam = floor($selisih / 3600);
                $waktu_format = $jam . " jam yang lalu";
            } else if ($selisih < 172800) {
                $waktu_format = "Kemarin, " . date('H:i', $timestamp);
            } else {
                $waktu_format = date('d/m/Y, H:i', $timestamp);
            }
    ?>
    <div class="activity-item">
        ...
        <div class="activity-details">
            <h4><?php echo $row_aktivitas['aksi']; ?></h4>
            <p>Oleh: <?php echo $row_aktivitas['username']; ?></p>
            <span class="activity-time"><?php echo $waktu_format; ?></span>
        </div>
    </div>
    <?php 
        }
...
// Sidebar toggle functionality yang diperbaiki
document.addEventListener('DOMContentLoaded', function() {
    const sidebar = document.querySelector('.sidebar');
    const content = document.querySelector('.content');
    const container = document.querySelector('.container');
    const toggleSidebarBtn = document.getElementById('toggleSidebar');
    const sidebarToggleOutside = document.querySelector('.sidebar-toggle');
    
    // Fungsi untuk mengatur posisi content berdasarkan status sidebar
    function adjustContentMargin() {
        if (window.innerWidth <= 992) {
            // Mobile/Tablet: sidebar overlay, content tidak perlu margin
    ...
    // Fungsi untuk toggle sidebar (tombol dalam sidebar)
    ...
    // Fungsi untuk toggle sidebar mobile (tombol luar)
    ...
    // Event listener untuk tombol toggle dalam sidebar
    if (toggleSidebarBtn) {
        toggleSidebarBtn.addEventListener('click', toggleSidebarCollapse);
    }
    
    // Event listener untuk tombol toggle luar (mobile)
    if (sidebarToggleOutside) {
        sidebarToggleOutside.addEventListener('click', toggleSidebarMobile);
    }
    ...
    document.addEventListener('click', function(event) {
        if (window.innerWidth <= 992 && 
            ...
            !sidebarToggleOutside.contains(event.target)) {
            sidebar.classList.remove('mobile-open');
            adjustContentMargin();
        }
    });
    ...
    setInterval(function() {
        // Hanya refresh bagian aktivitas, bukan seluruh halaman
        fetch(window.location.href)
            .then(response => response.text())
            .then(html => {
                const parser = new DOMParser();
                const newDoc = parser.parseFromString(html, 'text/html');
                const newActivityLog = newDoc.querySelector('.activity-log');
                const currentActivityLog = document.querySelector('.activity-log');
                
                if (newActivityLog && currentActivityLog) {
                    currentActivityLog.innerHTML = newActivityLog.innerHTML;
                }
            })
